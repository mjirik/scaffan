...
def get_image_properties(dataset_directory):
    ...
    while True:
        ...
        image_dictionary = {"id": image_name,
                            "width": width,
                            "height": height,
...
def get_category_properties(dataset_directory, filename):
    ...
    for i in range(number_lines):
        ...
        category_dictionary = {"supercategory": supercategory,
                               "id": i+1,
                               "name": supercategory
                              }
...
def get_annotations_properties(czi_files_directory, annotation_name): # TODO
    ...
    while True:
        filename_string = str(czi_files_directory) + "\\" + annotation_name + str(index) + ".jpg"
        filename_path = Path(filename_string)
        if not filename_path.exists():
            break
...
data = {}
info_dictionary = {"year": str(date.today().year),
                   "version": 1.0,
...
list_category_dictionaries = get_category_properties(dataset_directory, "categories.txt") # directory and .txt file
...
list_category_dictionaries = get_annotations_properties(czi_files_directory, annotation_name)
