...
experiment_title = "compare ndp and czi"

experiment_datetime = datetime.datetime.now()
experiment_datetime_fn = experiment_datetime.strftime("%Y%m%d-%H%M%S")
experiment_dir = Path(f"SA_{experiment_datetime_fn}_compare_ndp_and_czi")
...
def set_same(mainapp, fn, color):
    logger.debug(f"fn exists {Path(fn).exists()}, fn: {fn}, color={color}")
